...
export function MatchScoreForm({
  ...
  async function handleSave() {
    ...
    await supabase.from("audit_log").insert({
      ...
      actor_user_id: (await supabase.auth.getSession()).data.session?.user?.id ?? null,
      match_id: match.id,
      action: "score_updated",
      old_value: `${match.home_score}-${match.away_score} (${match.status})`,
      new_value: `${homeScore}-${awayScore} (${status})`,
    });

    router.push("/admin?tab=scoring");
    router.refresh();
  }

  const teamColor = (team: Team | null) => team?.color ?? "var(--chalk-dim)";
    ...
    <div className="max-w-md mx-auto space-y-6">
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-4">
          {/* Home score */}
          <div>
            <label className="text-xs block mb-2 text-center" style={{ color: "var(--chalk-dim)", fontFamily: "var(--font-mono)" }}>
              {homeTeam?.name ?? "Home"}
            </label>
            <div className="flex items-center justify-center gap-3">
              ...
              >−</button>
              <span style={{ fontFamily: "var(--font-display)", fontSize: "36px", color: "var(--chalk)", minWidth: "32px", textAlign: "center" }}>
                {homeScore}
              </span>
              <button
        ...
        <div>
          ...
          <select value={status} onChange={(e) => setStatus(e.target.value as Match["status"])} className={INPUT}>
